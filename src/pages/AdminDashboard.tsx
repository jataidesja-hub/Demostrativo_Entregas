...
function ProductsTab() {
  ...
  const openNew = () => {
    setEditing(null);
    setForm({ name: '', description: '', price: '', stock: '', category: categories[0]?.name || '', imageUrl: '' });
          ...
          <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
            {filteredProducts.map(p => (
              <tr key={p.id} className="text-zinc-700 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                ...
                <td className="p-4"><span className="bg-zinc-100 dark:bg-zinc-800 px-3 py-1 rounded-full text-xs font-bold">{p.category}</span></td>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Editar Produto' : 'Novo Produto'}>
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <InputField label="Preço de Venda (R$)" type="number" step="0.01" value={form.price} onChange={e => setForm(f => ({ ...f, price: e.target.value }))} placeholder="0.00" />
          ...
          <div className="space-y-2">
            ...
            {form.imageUrl && !imagePreviewError && (
              <div className="mt-2 rounded-xl overflow-hidden border border-zinc-200 dark:border-zinc-800 relative group">
                <img
                  src={form.imageUrl}
                  alt="Preview"
                ...
                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 transition-all flex items-center justify-center opacity-0 group-hover:opacity-100">
                  <span className="bg-white/90 dark:bg-zinc-900/90 px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1.5">
                    <Eye className="w-3.5 h-3.5" /> Preview da imagem
                  </span>
                </div>
              </div>
            )}
    ...
    <div className="space-y-6">
      <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-100 dark:border-zinc-800 shadow-sm p-6">
        ...
        <div className="flex gap-4 items-end">
          ...
          <Button onClick={handleAdd} className="h-12 rounded-xl"><Plus className="w-5 h-5 mr-2" /> Adicionar</Button>
        </div>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {categories.map(c => (
          <div key={c.id} className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-100 dark:border-zinc-800 p-5 flex items-center justify-between group hover:shadow-md transition-all">
            <div className="flex items-center gap-3">
