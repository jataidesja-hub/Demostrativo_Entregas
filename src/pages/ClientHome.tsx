...
import {
  ...
  Download, Smartphone, ClipboardList
} from "lucide-react";
  ...
  return (
    <>
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-[2.5rem] w-full max-w-lg overflow-hidden relative shadow-2xl animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-8">
              ...
              <div className="flex items-center justify-between py-6 my-6 border-y">
                <span className="text-4xl font-black italic" style={{ color: config.primaryColor }}>{formatCurrency(getPromoPrice(viewingProduct.id, viewingProduct.price) ?? viewingProduct.price)}</span>
                <span className={`text-xs font-black uppercase ${(viewingProduct.stock ?? 0) <= 0 ? 'text-red-500' : 'text-emerald-500'}`}>{(viewingProduct.stock ?? 0) <= 0 ? 'Esgotado' : `${viewingProduct.stock} un.`}</span>
              </div>
      ...
      <a
        ...
        className="fixed bottom-24 right-6 w-16 h-16 bg-[#25D366] text-white rounded-full flex items-center justify-center shadow-xl active:scale-75 transition-all z-[400] border-4 border-white dark:border-zinc-900"
      >
